...
const BADGE_DEFINITIONS = {
  ...
  "algorithm-expert": {
    ...
    condition: (user) =>
      user.solvedProblems.filter(
        (p) => p.topic === "Dynamic Programming" || p.topic === "Greedy"
      ).length >= 10,
  },
  "frontend-wizard": {
    ...
    condition: (user) =>
      user.solvedProblems.filter(
        (p) => p.topic === "Array" || p.topic === "String"
      ).length >= 5,
  },
  "backend-architect": {
  ...
  "array-master": {
    ...
    category: "skill",
    condition: (user) =>
      user.solvedProblems.filter((p) => p.topic === "Array").length >= 10,
  },
  "recursion-king": {
    name: "Recursion King",
    ...
    category: "skill",
    condition: (user) =>
      user.solvedProblems.filter((p) => p.topic === "Recursion").length >= 8,
  },
  "speed-demon": {
    name: "Speed Demon",
    ...
    condition: (user) => {
      ...
      return (
        user.solvedProblems.filter(
          (p) => p.solvedAt >= today && p.solvedAt < tomorrow
        ).length >= 10
      );
    },
  },
  perfectionist: {
    ...
    category: "milestone",
    condition: (user) =>
      user.solvedProblems.length >= 50 && user.stats.accuracy >= 100,
  },
};
...
router.post("/purchase", authenticateToken, async (req, res) => {
  try {
    ...
    const alreadyOwned = user.rewards.ownedItems.some(
      (owned) => owned.itemId === itemId
    );
    if (alreadyOwned) {
...
router.post("/activate-booster", authenticateToken, async (req, res) => {
  try {
    ...
    const ownedItem = user.rewards.ownedItems.find(
    ...
    const activeBooster = user.rewards.activeBoosters.find(
      (booster) => booster.boosterId === itemId
    );
    if (activeBooster) {
...
router.post("/claim-daily", authenticateToken, async (req, res) => {
  try {
    ...
    if (!canClaim) {
      return res
        .status(400)
        .json({ message: "Daily reward already claimed today" });
    }
    ...
    user.rewards.dailyRewards.nextReward = Math.min(
      user.rewards.dailyRewards.nextReward + 1,
      7
    );
...
router.get("/leaderboard", async (req, res) => {
  try {
    ...
    const users = await User.find({ "rewards.coins": { $exists: true } })
      .select(
        "username fullName rewards.coins rewards.xp rewards.badges stats.longestStreak"
      )
      .sort({ [sortField]: -1 })
      .limit(10);
...
async function checkAndAwardBadges(user) {
  for (const [badgeId, badgeDef] of Object.entries(BADGE_DEFINITIONS)) {
    const alreadyEarned = user.rewards.badges.some(
...
async function checkAndUpdateAchievements(user) {
  for (const [achievementId, achievementDef] of Object.entries(
    ACHIEVEMENT_DEFINITIONS
  )) {
    let existingAchievement = user.rewards.achievements.find(
